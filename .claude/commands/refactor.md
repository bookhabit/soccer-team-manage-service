...
### 6. ERD 동기화 (서버 코드 변경 시 필수)
`server/prisma/schema.prisma` 를 읽어 `docs/erd.md` 와 비교한다.

- [ ] 이번 기능 구현으로 스키마에 추가/변경된 모델이 있는가?
- 있으면 → `docs/erd.md` 를 스키마 기준으로 **전체 재생성**한다
- 없으면 → "✅ 스키마 변경 없음 — ERD 동기화 불필요" 로 표시한다

ERD 생성 규칙:
- Mermaid `erDiagram` 형식 사용
- `@id` → `PK`, 외래키 → `FK`, `@unique` → `UK`
- 관계: 1:N `||--o{`, 1:1 `||--||`, N:M `}o--o{`, optional `|o`
- Enum 정의 표 및 모델 요약 표 포함
- 파일 상단에 마지막 동기화 날짜 기록
