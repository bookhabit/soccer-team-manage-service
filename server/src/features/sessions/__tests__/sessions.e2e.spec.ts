...
function extractRtCookie(res: Response): string | undefined {
  ...
  const rtEntry = cookies.find((c) => c.startsWith("soccer_app_rt="));
...
describe("Sessions E2E — /api/v1/sessions", () => {
  ...
  describe("POST /api/v1/sessions (로그인)", () => {
    ...
    it("잘못된 비밀번호 → 401 + INVALID_CREDENTIALS", async () => {
      ...
      expect(res.body.code ?? res.body.message).toContain(
        "INVALID_CREDENTIALS",
      );
    });
  ...
  describe("POST /api/v1/sessions/refresh (Silent Refresh)", () => {
    it("유효한 RT 쿠키 → 200 + 새 AT + 새 RT 쿠키 (RTR)", async () => {
      ...
      const refreshRes = await request(app.getHttpServer())
        .post(REFRESH_URL)
        .set("Cookie", `soccer_app_rt=${oldRt}`)
        .expect(200);
    ...
    it("위조된 RT → 401 Unauthorized", async () => {
      await request(app.getHttpServer())
        .post(REFRESH_URL)
        .set("Cookie", "soccer_app_rt=forged.token.value")
        .expect(401);
    });
    ...
    it("RTR 재사용 감지 후 새 RT도 무효화 (세션 삭제 확인)", async () => {
      ...
      await request(app.getHttpServer())
        .post(REFRESH_URL)
        .set("Cookie", `soccer_app_rt=${newRt}`)
        .expect(401);
    });
  });
  ...
  describe("DELETE /api/v1/sessions (로그아웃)", () => {
    it("정상 로그아웃 → 204 + RT 쿠키 삭제", async () => {
      ...
      const setCookie = logoutRes.headers["set-cookie"] as
        | string[]
        | string
        | undefined;
      if (setCookie) {
        const cookies = Array.isArray(setCookie) ? setCookie : [setCookie];
        const rtEntry = cookies.find((c) => c.includes("soccer_app_rt"));
        if (rtEntry) {
  ...
  describe("AT 인가 검증", () => {
    it("만료된 AT 형식 → 401 Unauthorized", async () => {
      ...
      await request(app.getHttpServer())
        ...
        .set(
          "Authorization",
          "Bearer eyJhbGciOiJIUzI1NiJ9.eyJzdWIiOiIxMjMifQ.invalid",
        )
        .expect(401);
    });
