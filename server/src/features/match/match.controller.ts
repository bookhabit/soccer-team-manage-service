...
export class MatchController {
  ...
  @ApiResponse({
    status: 403,
    description: "CLUB_NO_PERMISSION — 주장·부주장 아님",
  })
  createMatch(
  ...
  @ApiResponse({
    ...
    description: "내 투표 응답·참석/불참 수 포함",
  })
  ...
  @ApiResponse({
    status: 422,
    description: "MATCH_009 — 투표 마감 후 수정 불가",
  })
  updateMatch(
    @CurrentUser() user: JwtPayload,
    @Param("clubId") clubId: string,
  ...
  @ApiOperation({ summary: "경기 참석 투표 제출 (upsert)" })
  @ApiResponse({
    status: 201,
    type: SubmitAttendanceResponseDto,
    description: "투표 응답 저장 (중복 제출 시 덮어쓰기)",
  })
  @ApiResponse({ status: 403, description: "CLUB_NO_PERMISSION — 클럽 미소속" })
  @ApiResponse({ status: 404, description: "MATCH_001 — 존재하지 않는 경기" })
  @ApiResponse({
    status: 422,
    description: "MATCH_003 — 투표 마감 후 변경 불가",
  })
  submitAttendance(
    @CurrentUser() user: JwtPayload,
    @Param("clubId") clubId: string,
  ...
  @ApiResponse({
    status: 200,
    type: [AttendanceItemResponseDto],
    description: "전체 팀원 투표 응답 목록",
  })
  @ApiResponse({ status: 403, description: "CLUB_NO_PERMISSION — 클럽 미소속" })
  @ApiResponse({ status: 404, description: "MATCH_001 — 존재하지 않는 경기" })
  getAttendances(
  ...
  @ApiResponse({
    status: 200,
    type: [QuarterResponseDto],
    description: "쿼터 번호·포메이션·팀·선수 배정 목록",
  })
  @ApiResponse({ status: 403, description: "CLUB_NO_PERMISSION — 클럽 미소속" })
  @ApiResponse({ status: 404, description: "MATCH_001 — 존재하지 않는 경기" })
  getLineup(
  ...
  @ApiResponse({
    status: 404,
    description:
      "MATCH_001 — 존재하지 않는 경기 / MATCH_010 — 참여 선수 목록에 없는 유저",
  })
  saveLineup(
    @CurrentUser() user: JwtPayload,
    @Param("clubId") clubId: string,
  ...
  @ApiResponse({
    ...
    description: "참여 선수 추가 성공 (중복 추가 시 무시)",
  })
  @ApiResponse({
    status: 403,
    description:
      "CLUB_NO_PERMISSION — 주장·부주장 아님 또는 대상이 클럽 미소속",
  })
  @ApiResponse({ status: 404, description: "MATCH_001 — 존재하지 않는 경기" })
  addParticipant(
    @CurrentUser() user: JwtPayload,
    ...
    return this.matchService.addParticipant(
      clubId,
      matchId,
      user.sub,
      dto.userId,
  ...
  removeParticipant(
    ...
    return this.matchService.removeParticipant(
      clubId,
      matchId,
      user.sub,
      targetId,
  ...
  @ApiResponse({
    status: 422,
    description: "MATCH_004 — 경기 종료 전 입력 불가",
  })
  submitRecord(
    @CurrentUser() user: JwtPayload,
    @Param("clubId") clubId: string,
  ...
  @ApiResponse({
    status: 200,
    type: RecordResponseDto,
    description: "수정 전·후 이력 저장 후 최신 기록 반환",
  })
  @ApiResponse({
    status: 403,
    description: "CLUB_NO_PERMISSION — 주장·부주장 아님",
  ...
  @ApiResponse({
    status: 200,
    type: [RecordHistoryItemResponseDto],
    description: "최신순 변경 이력 목록",
  })
  @ApiResponse({
    status: 403,
    description: "CLUB_NO_PERMISSION — 주장·부주장 아님",
  ...
  @ApiResponse({
    status: 201,
    type: MomVoteResponseDto,
    description: "MOM 투표 저장 성공",
  })
  @ApiResponse({ status: 403, description: "CLUB_NO_PERMISSION — 클럽 미소속" })
  @ApiResponse({ status: 404, description: "MATCH_001 — 존재하지 않는 경기" })
  ...
  @ApiResponse({
    status: 422,
    description: "MATCH_004 — 기록 미등록 / MATCH_005 — 투표 마감",
  })
  submitMomVote(
    @CurrentUser() user: JwtPayload,
    @Param("clubId") clubId: string,
  ...
  @ApiResponse({
    status: 200,
    type: MomResultResponseDto,
    description: "득표수 내림차순 정렬, 공동 수상자 모두 포함",
  })
  @ApiResponse({ status: 403, description: "CLUB_NO_PERMISSION — 클럽 미소속" })
  @ApiResponse({ status: 404, description: "MATCH_001 — 존재하지 않는 경기" })
  getMomResult(
  ...
  @ApiResponse({
    status: 404,
    description: "MATCH_001 — 존재하지 않는 경기 또는 댓글",
  })
  deleteComment(
    @CurrentUser() user: JwtPayload,
    @Param("clubId") clubId: string,
    ...
    return this.matchService.deleteComment(
      clubId,
      matchId,
      commentId,
      user.sub,
  ...
  @ApiResponse({
    status: 404,
    description: "MATCH_001 — 존재하지 않는 경기 또는 영상",
  })
  deleteVideo(
    @CurrentUser() user: JwtPayload,
    @Param("clubId") clubId: string,
  ...
  @ApiResponse({
    status: 201,
    type: OpponentRatingResponseDto,
    description: "상대팀 평가 저장 성공",
  })
  @ApiResponse({ status: 403, description: "CLUB_NO_PERMISSION — 클럽 미소속" })
  @ApiResponse({ status: 404, description: "MATCH_001 — 존재하지 않는 경기" })
  ...
  @ApiResponse({
    status: 422,
    description: "MATCH_007 — 매칭전 아님 또는 기록 미등록",
  })
  submitOpponentRating(
    ...
    return this.matchService.submitOpponentRating(
      clubId,
      matchId,
      user.sub,
      dto,
