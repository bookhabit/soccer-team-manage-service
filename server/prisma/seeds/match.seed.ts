...
async function seedAttendances() {
  ...
  const deadlinePassedAttendances = [
    {
      matchId: MATCH.BEFORE_DEADLINE_PASSED,
      userId: USER.CAPTAIN,
      response: AttendanceResponse.ATTEND,
    },
    {
  ...
  const afterNoRecordAttendances = [
    {
      matchId: MATCH.AFTER_SELF_NO_RECORD,
      userId: USER.CAPTAIN,
      response: AttendanceResponse.ATTEND,
    },
    {
  ...
  const kadongAttendances = [
    {
      matchId: MATCH.KADONG_BEFORE,
      userId: USER.KADONG_CAPTAIN,
      response: AttendanceResponse.ATTEND,
    },
  ];
  ...
  console.log(`  ✅ 경기 참석 투표 ${all.length}건 완료`);
}
...
async function seedGoals() {
  ...
  await prisma.matchGoal.deleteMany({
    where: {
      matchId: { in: [MATCH.AFTER_LEAGUE_RECORDED, MATCH.AFTER_MOM_ACTIVE] },
    },
  });
...
async function seedMomVotes() {
  ...
  const recordedVotes = [
    {
      matchId: MATCH.AFTER_LEAGUE_RECORDED,
      voterId: USER.CAPTAIN,
      targetUserId: USER.MEMBER1,
    },
    {
      matchId: MATCH.AFTER_LEAGUE_RECORDED,
      voterId: USER.VICE,
    ...
    {
      matchId: MATCH.AFTER_LEAGUE_RECORDED,
      voterId: USER.MEMBER1,
      targetUserId: USER.CAPTAIN,
    },
    {
      matchId: MATCH.AFTER_LEAGUE_RECORDED,
      voterId: USER.MEMBER2,
      targetUserId: USER.CAPTAIN,
    },
  ];
...
async function main() {
  ...
  console.log(
    "  [BEFORE - 자체전]    마무리FC 자체 훈련전 (3일 후, 투표 가능)",
  );
  console.log(
    "  [BEFORE - 매칭전]    마무리FC vs 카동FC 정기전 (5일 후, 투표 가능)",
  );
  console.log("  [BEFORE - 마감]      정기 훈련전 (2일 후, 투표 마감됨)");
  console.log("  [DURING]             지금 진행중인 경기");
  console.log(
    "  [AFTER - 기록완료]   지난 주 정기전 (3:1, 골기록·MOM결과 있음)",
  );
  console.log(
    "  [AFTER - 기록없음]   지난 훈련전 (주장에게 기록입력 CTA 노출)",
  );
  console.log(
    "  [AFTER - MOM진행중]  2주 전 정기전 (주장만 투표, 멤버는 미투표)",
  );
  console.log(
    "────────────────────────────────────────────────────────────────",
  );
  console.log("\n🧪 검증 시나리오:");
  console.log(
    "  captain@mamurifc.test → 경기 생성/수정, 라인업, 기록입력 버튼 노출 확인",
  );
  console.log(
    "  member1@mamurifc.test → 투표 버튼 노출, MOM 투표 가능 상태 확인",
  );
