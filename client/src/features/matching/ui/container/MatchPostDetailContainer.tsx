...
import { MatchPostDetailView } from '../view/MatchPostDetailView';
...
interface MatchPostDetailContainerProps {
  postId: string;
}

function MatchPostDetailContent({ postId }: MatchPostDetailContainerProps) {
...
function MatchPostDetailSkeleton() {
  return (
    <ScreenLayout>
...
/**
 * 매칭 게시글 상세 Container — 신청 BottomSheet + 전화번호 가드 포함.
 * 클럽 내부 경기 상세는 ClubMatchDetailContainer를 사용.
 */
export function MatchPostDetailContainer({ postId }: MatchPostDetailContainerProps) {
  return (
    <AsyncBoundary loadingFallback={<MatchPostDetailSkeleton />}>
      <MatchPostDetailContent postId={postId} />
    </AsyncBoundary>
  );
}
