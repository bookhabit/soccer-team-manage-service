...
THEN   해당 신청 ACCEPTED, 나머지 PENDING → REJECTED 일괄 처리
       게시글 status → MATCHED
       양측 관리자에게 연락처 인앱 표시
       [기획 변경] Match 자동 생성 제거 — 경기 등록 시 매칭 정보 pre-fill 편의로 대체
...
| `PATCH`  | `/match-posts/:id/cancel`                     | 매칭 취소 (MATCHED → CANCELLED)      | 등록팀 관리자       |
...
status,               // 'OPEN' | 'MATCHED' | 'CANCELLED' (EXPIRED는 isExpired 파생)
...
cancelMatchPost(id); // PATCH /match-posts/:id/cancel
...
useCancelMatchPost(); // useMutation — MATCHED → CANCELLED
```
...
| `components/MatchFilterBar.tsx`   | 필터 바 (날짜/지역/레벨/성별/구장비)                        |
...
- [x] `accept` — 트랜잭션 (ACCEPTED + 일괄 REJECTED + status MATCHED). Match 자동생성 제거.
